![3d Game Shaders For Beginners](https://i.imgur.com/avBSxHD.gif)
...
Each [Blender](https://blender.org) material used to build `mill-scene.egg` has three textures.
...
The develop the example code, `gl-coordinate-system default`, `textures-power-2 down`,
and `textures-auto-power-2 1` were added to `Config.prc`.
...
Note that Panda3D works on Linux, Mac, and Windows.
...
You can now run the example code like so.
...
Note the two keywords `uniform` and `in`.
...
Kind in mind that a fragment affects at most one screen pixel but a single pixel can be affected by many fragments.

Note the keyword `out`.
...
Because of this, you can calculate the UV coordinates knowing only the width and height of the screen sized texture being
UV mapped to the rectangle and the fragment's x and y coordinate.
...
Note that if the diffuse intensity is zero or less, move on to the next light.
...
Note how I clamp the diffuse color to be only as bright as the diffuse texture color is.
...
Note that you must negate `unitLightDirection`
because it goes from the fragment to the spotlight and you need it to go from the spotlight to the fragment
...
Note that `textureProj` can also return a value between zero and one depending on how the shadow map was set up.
...
Note that you'll revisit the ambient color when performing SSAO.
...
<br>
<br>
<footer>[Source](http://www.panda3d.org/manual/?title=List_of_GLSL_Shader_Inputs)</footer>
</blockquote>
...
This is the material diffuse framebuffer texture.
The outline shader will detect the edges in the scene and color them in.
Note that using the material diffuse color won't work unless
the distinctive pieces of the scene have their own material diffuse color.
The mill scene's material diffuse colors were set in Blender.
...
Note that "screen space" ambient occlusion is a bit of a misnomer as not all of the calculations are done in screen space.
...
<br>
<footer>
[Source](https://www.khronos.org/registry/OpenGL/specs/gl/glspec44.core.pdf)
</footer>
</blockquote>
...
Now weight this sample by how far it is inside or outside the radius.
...
Divide the occluded count by the number of samples to give you the average number of occluded samples.
This is the occlusion factor.
Assign the occlusion factor to the fragment's color and you're done.

Note that the example code sets the alpha channel to the position framebuffer
texture's alpha to avoid occluding the background.
